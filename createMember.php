<?php
    ...
    function check_pw(){
        if (preg_match("/(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}/", htmlspecialchars($_POST["passwd"])) && strlen($_POST["passwd"]) <= 25)
            return true;
        else{
            global $error;
            $error = 1;
    ...
    function check_log(){
        ...
        $login = $req->fetch();
        if ($login["login"] || strlen($_POST["login"]) > 25){
            global $error;
            $error = 3;
            return false;
        }
    ...
    function check_email(){
        ...
        $mail = $req->fetch();
        if ($mail["email"] || strlen($_POST["mail"]) > 40){
            global $error;
            $error = 4;
            return false;
        }
    ...
    if (isset($_POST["passwd"]) && isset($_POST["mail"]) && isset($_POST["login"]) && isset($_POST["submit"]) &&
        htmlspecialchars($_POST["submit"]) == "Creer un compte" && htmlspecialchars($_POST["login"]) &&
